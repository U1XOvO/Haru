...
const drafts={chat:'',grammar:'わたしは中国人です。',word:'',topic:'街角的咖啡店'};
...
async function saveAIConfig(form,testConnection){
 ...
 const data=new FormData(form),result=form.querySelector('#ping-result');
 const params={base:data.get('base'),model:data.get('model'),key:data.get('key'),timeout:Number(data.get('timeout'))};
 fields.disabled=true;result.textContent='正在保存配置…';let saved=false;
 await run(testConnection?'正在保存并测试 AI 连接…':'正在保存 AI 配置…',async()=>{
  try{
   const c=await rpc('config_save',params);saved=true;
   form.elements.key.value='';form.elements.key.required=!c.key_configured;form.elements.key.placeholder=c.key_configured?'已配置 · 留空保留原密钥':'填写你的 API 密钥';
   if(form.isConnected)aiConfig=c;
   result.textContent=c.overrides.length?'已保存到 .env；当前仍优先使用启动环境变量。':'已保存到 .env，后续请求自动使用。';
