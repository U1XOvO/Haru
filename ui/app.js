...
function aiProviderView(c,index){
 ...
 <div class="ai-provider-header"><div class="ai-provider-identity"><span class="ai-provider-number">${String(index+1).padStart(2,'0')}</span><div><h4>${esc(c.name)}</h4><span class="ai-provider-caption">${esc(c.model||'填写连接信息即可开始')}</span></div></div>
...
function aiConfigView(){
 ...
 <details class="ai-defaults" open><summary>默认调用参数与覆盖规则</summary><div class="ai-defaults-body">
 <div class="ai-table-wrap"><table><thead><tr><th>功能</th><th>temperature</th><th>max_tokens</th></tr></thead><tbody>
 <tr><td>普通教学、词卡、注音、周测、沉浸故事、连接测试</td><td>0.55</td><td>5000</td></tr>
...
<label style="margin:20px 0;display:grid;gap:8px">
  <span>日语朗读语速 <b id="speech-rate-display">${(p.speech_rate??1.0).toFixed(2)}×</b></span>
...
document.addEventListener('input',e=>{  if(e.target.matches('[data-speech-rate]')){
    const v=Number(e.target.value).toFixed(2);
    $('#speech-rate-display').textContent=v+'×';
    return;
