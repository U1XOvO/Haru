...
let studyAnswerSequence=0;
const studyPendingAnswers=new Map();
...
function studyRememberAttempt(a){
 studyAttempt=a;studySaveFailed=false;studyPendingAnswers.clear();
...
function studyAnswers(a=studyAttempt){
 const answers={...a.answers};
 for(const [id,pending] of studyPendingAnswers)if(pending.attempt===a.id)answers[id]=pending.value;
 return answers;
}
function studyMoreButton(kind){
 const info=studyCatalog?.[kind==='papers'?'papers_page':'history_page'];
 return info?.next_offset!=null?studyButton(`加载更多（共 ${info.total} 项）`,kind+'-more',`data-offset="${info.next_offset}"`):'';
}
...
function studyHistoryHTML(){return `<section class="card"><h2>${examLevel} 作答记录</h2><div class="history-list">${studyCatalog.history.map(h=>`<button type="button" data-study="resume" data-id="${h.id}"><b>${esc(h.title)}</b><small>${new Date(h.started*1000).toLocaleString('zh-CN')} · ${h.status==='submitted'?h.correct+'/'+h.total+' 正确':'已答 '+h.answered+'/'+h.total} · ${h.mode==='timed'?'计时':'练习'} · ${h.status==='submitted'?'查看解析':'继续作答'}</small></button>`).join('')||'<p class="hint">完成第一组题目后，这里会留下记录。</p>'}</div>${studyMoreButton('history')}</section>`;}
...
function studyAttemptView(){
 const a=studyAttempt,p=a.paper,answers=studyAnswers(a),done=a.status==='submitted',q=p.questions[studyQuestion]||p.questions[0];
 ...
 `<div class="study-attempt-tools">${studyButton('返回目录','back')}<span class="pill" id="study-answer-count">${done?'已交卷':'已答 '+Object.values(answers).filter(v=>v!==null).length+'/'+p.questions.length}</span>${!done&&a.mode==='timed'?'<strong id="study-clock" aria-live="off"></strong>':''}<span id="study-save-status" role="status">${studySaveFailed?'保存失败，请重新打开记录':'已保存在本机'}</span>${studyButton('重新读取已保存作答','reload-attempt')}</div>`+
 ...
 `<div class="exam-layout"><aside class="card exam-palette"><h3>答题卡</h3><div class="study-section-list">${p.sections.map((s,i)=>`<span class="${q.section===s.id?'active':''}">${esc(s.title)}${a.mode==='timed'?' · '+Math.round(s.seconds/60)+'分钟':''}</span>`).join('')}</div><div class="answer-palette">${visible.map(([x,i])=>`<button type="button" data-study="question" data-index="${i}" class="${i===studyQuestion?'current ':''}${answers[x.id]!==undefined&&answers[x.id]!==null?'answered ':''}${a.flags.includes(x.id)?'flagged ':''}${done&&!a.result.results[i].correct?'incorrect':''}" aria-label="第${i+1}题${a.flags.includes(x.id)?' 已标记':''}">${i+1}${a.flags.includes(x.id)?'★':''}</button>`).join('')}</div><p class="hint">着色：已答 · ★：待检查</p>${!done?`<div class="stack">${a.mode==='timed'?studyButton('提交当前分区','next-section','','soft'):''}${studyButton('交卷并查看结果','submit','','primary')}</div>`:''}</aside><article class="card exam-question"><div class="card-title"><span class="pill">第 ${studyQuestion+1} 题 · ${esc(q.type_title||studySkills[q.skill])}</span>${!done?studyButton(a.flags.includes(q.id)?'★ 已标记':'☆ 标记待检查','flag'):''}</div>${p.source_type==='official'?`<div class="callout"><strong>对照官方原卷作答</strong><p>定位：${esc(q.locator)}。请在原卷中阅读题干与选项，再选择对应编号。</p></div>`:''}${p.pdf_asset?studyButton('打开原卷 PDF','asset',`data-id="${p.pdf_asset}"`):''}${q.passage?`<div class="study-passage" lang="ja">${done||a.mode==='practice'?japanese(q.passage):esc(q.passage)}</div>`:''}${q.image_asset?`<div class="study-image" data-study-image="${q.image_asset}">${studyImages.has(q.image_asset)?`<img src="${studyImages.get(q.image_asset)}" alt="本题配图">`:studyButton('加载题图','image',`data-id="${q.image_asset}"`)}</div>`:''}${q.audio_asset?studyButton('播放原音频','asset',`data-id="${q.audio_asset}"`):''}${q.audio_text?`<div class="callout">${speak(q.audio_text,true)} <small>模拟听力 · Edge TTS 合成语音</small>${done||a.mode==='practice'?`<details><summary>听力原文</summary><p>${esc(q.audio_text)}</p></details>`:''}</div>`:''}${q.audio_asset||q.audio_text?studyButton('停止播放','stop-audio'):''}<h2 class="study-prompt" lang="ja">${esc(q.prompt)}</h2><div class="study-options">${q.options.map((o,i)=>`<label class="option ${answers[q.id]===i?'chosen':''} ${done&&q.answer===i?'correct':''}"><input type="radio" name="study-answer" value="${i}" ${answers[q.id]===i?'checked':''} ${done?'disabled':''}><span><b>${String.fromCharCode(65+i)}.</b> ${esc(o)}</span></label>`).join('')}</div>${done?`<div class="result-line ${feedback.correct?'':'wrong'}"><strong>${feedback.correct?'✓ 回答正确':feedback.selected===null?'未作答':'需要再看一眼'} · 正确选项 ${String.fromCharCode(65+q.answer)}</strong><p>${esc(q.explanation)}</p>${q.grammar_ids.map(id=>studyButton('学习关联语法','linked-grammar',`data-id="${id}"`)).join('')}</div>`:''}<div class="study-question-nav">${studyButton('上一题','prev-question')}${studyButton('下一题','next-question')}</div>${resources.length?`<details class="study-provenance" ${p.source_type==='official'?'open':''}><summary>原卷与资料 · 官方 JLPT 网站</summary><div class="inline-actions">${resources.map(r=>`<button type="button" class="text-link" data-url="${esc(r.url)}">${esc(r.title)} ↗</button>`).join('')}</div></details>`:''}<p class="hint">${esc(p.source||'')}${p.model?' · 实际模型 '+esc(p.model):''} · 版本 ${esc(p.version)}</p></article></div>`;
}
function studyUpdateAttemptDOM(){
 const a=studyAttempt;if(!a||a.status!=='active'||!['jlpt','grammar_library'].includes(page))return;
 const answers=studyAnswers(a),q=a.paper.questions[studyQuestion];
 document.querySelectorAll('input[name="study-answer"]').forEach(input=>{
  const chosen=answers[q.id]===Number(input.value);input.checked=chosen;input.closest('.option').classList.toggle('chosen',chosen);
 });
 document.querySelectorAll('.answer-palette [data-index]').forEach(button=>{
  const question=a.paper.questions[Number(button.dataset.index)];
  button.classList.toggle('answered',answers[question.id]!==undefined&&answers[question.id]!==null);
 });
 const count=$('#study-answer-count');if(count)count.textContent='已答 '+Object.values(answers).filter(v=>v!==null).length+'/'+a.paper.questions.length;
}
async function studyAcceptState(result,id){
 if(studyAttempt?.id!==id)return result;
 const before=studyAttempt;
 let next=result.delta?{...before,...result,paper:before.paper}:result;
 // Answers and explanations are fetched once, only when submission reveals them.
 if(next.status==='submitted'&&before.status!=='submitted')next=await rpc('study_attempt',{id});
 if(studyAttempt?.id!==id)return next;
 const transitioned=before.status!==next.status||before.section_index!==next.section_index;
 delete next.delta;studyAttempt=next;
 if(transitioned){studyRememberAttempt(next);if(['jlpt','grammar_library'].includes(page))render();}
 return next;
}
async function studyWrite(patch={}){
 const id=studyAttempt?.id,sequence=++studyAnswerSequence;
 for(const [question,value] of Object.entries(patch.answers||{}))studyPendingAnswers.set(question,{attempt:id,value,sequence});
 studyUpdateAttemptDOM();
 const operation=async()=>{
  if(!studyAttempt||studyAttempt.id!==id)throw new Error('作答已切换，请重新打开。');
  if(studySaveFailed)throw new Error('上次保存失败，请先重新读取已保存作答。');
  try{
   if($('#study-save-status'))$('#study-save-status').textContent='正在保存…';
   const result=await rpc('study_save',{id,revision:studyAttempt.revision,...patch}),a=await studyAcceptState(result,id);
   for(const question of Object.keys(patch.answers||{}))if(studyPendingAnswers.get(question)?.sequence===sequence)studyPendingAnswers.delete(question);
   if(studyAttempt?.id===id){studyUpdateAttemptDOM();if($('#study-save-status'))$('#study-save-status').textContent=studyPendingAnswers.size?'正在保存…':'已保存在本机';}
   return a;
...
async function studyAction(b){
 ...
 await run('正在处理学习记录…',async()=>{
  ...
  }else if(action==='papers-more'||action==='history-more'){
   const level=examLevel,kind=action==='papers-more'?'papers':'history',offset=Number(b.dataset.offset),catalogRun=studyReadStart('studyCatalog');
   const data=await rpc('study_catalog',{level,[kind==='papers'?'paper_offset':'history_offset']:offset});
   if(!actionCurrent()||!studyReadCurrent('studyCatalog',catalogRun)||examLevel!==level||!studyCatalog)return;
   const seen=new Set(studyCatalog[kind].map(item=>item.id));
   studyCatalog[kind].push(...data[kind].filter(item=>!seen.has(item.id)));
   studyCatalog[kind==='papers'?'papers_page':'history_page']=data[kind==='papers'?'papers_page':'history_page'];
  ...
  else if(action==='stop-audio'){if(haruHasNative())await rpc('study_stop_audio');else toast('语音播放请在 Haru 桌面 App 中使用。',true);return;}
...
document.addEventListener('change',e=>{
 ...
 if(e.target.name==='study-answer'){
  ...
  if(a.status!=='active'||studySaveFailed){studyUpdateAttemptDOM();return;}
  void studyWrite({answers:{[q.id]:selected}}).catch(e=>toast(e.message,true));
 }
});
...
setInterval(()=>{
 ...
 if(a.mode==='timed'){
  ...
  if(remain===0&&!studyClockFlight){
   studyClockFlight=true;
   const next=studyQueue.then(async()=>{const result=await rpc('study_attempt',{id:a.id,state_only:true});return studyAcceptState(result,a.id);});
   studyQueue=next.catch(()=>{});next.catch(e=>toast(e.message,true)).finally(()=>{studyClockFlight=false;});
  }
 }
},1000);
setInterval(()=>{if(studyAttempt?.status==='active'&&!document.hidden&&['jlpt','grammar_library'].includes(page)&&!studySaveFailed)void studyWrite().catch(e=>toast(e.message,true));},20000);
